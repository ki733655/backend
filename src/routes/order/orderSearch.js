...
router.get("/order-search-deliveryStatus", async (req, res) => {
    try {
        const data = req.query.search;
        const results = await Order.find({ deliveryStatus: { $regex: new RegExp(`^${data}`, 'i') } });
        ...
        console.error("Error searching for Delivery statsu:", err);
        res.status(500).json({ message: "Internal server error" }); // Send a 500 status if an error occurs
    }
});
